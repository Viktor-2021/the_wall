...
def home(request):
    return (render(request,'wall_app_/login.html'))
...
def wall(request):
    ...
    return render(request, 'wall_app_/wall.html', context)
    ...
    if request.method == 'GET':
        ...
        if request.method == 'POST':
            ...
            if len(errors) > 0:
                ...
                obj = Message.objects.create(user = user, message = message)
                obj.save()
                return redirect('/wall')
    ...
    if request.method == 'GET':
        ...
        if request.method == 'POST':
            ...
            if len(errors) > 0:
                ...
                obj = Comment.objects.create(user = user, message_id = message_id, comment = comment)
                obj.save()
                return redirect('/wall')